...
def prepare_export(ls: LabelStudio,
                   project_id: int,
    ...
    data_iter = ls.projects.exports.download(id=project_id,
                                             export_pk=export.id,
                                             export_type='JSON')
...
def run(url: str, api_key: str, project_id: int, output_dir: str):
    ...
    converter = Converter(config=label_config,
                          project_dir=os.path.dirname(snapshot_path),
                          download_resources=False)

    logger.info("Converting to YOLO format.")
    converter.convert_to_yolo(input_data=snapshot_path,
    ...
    for task in tqdm(exported_tasks):
        ...
        if image_url:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    local_image_path = get_local_path(url=image_url,
                                                      hostname=url,
                                                      access_token=api_key,
                                                      task_id=task['id'],
                                                      download_resources=True)
                    name = os.path.basename(local_image_path).split('__',
                                                                    1)[-1]
                    ...
                    logger.error(
                        f"Error downloading image for task {task['id']}: {e}")
...
if __name__ == "__main__":
    ...
    run(args.url, args.api_key, args.project_id, args.output_dir)
